...
public class PublicReportServiceImpl extends APIServiceImpl implements PublicReportService {
    ...
    @Override
    public void updateIncludeCoralsFlag(Long aquariumId, boolean includeCorals, String token) throws BusinessException {
        String uri = sabiBackendUrl + Endpoint.REPORT_LINK.getPath() + "/" + aquariumId
                     + "/include-corals?includeCorals=" + includeCorals;
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = RestHelper.prepareAuthedHttpHeader(token);
        HttpEntity<String> request = new HttpEntity<>(headers);
        try {
            ...
            log.error("Failed to update includeCorals for aquarium_id={}", aquariumId, e);
            throw new BusinessException(CommonExceptionCodes.NETWORK_ERROR);
        }
    }

    @Override
    public void updateIncludeInvertebratesFlag(Long aquariumId, boolean includeInvertebrates, String token) throws BusinessException {
        String uri = sabiBackendUrl + Endpoint.REPORT_LINK.getPath() + "/" + aquariumId
                     + "/include-invertebrates?includeInvertebrates=" + includeInvertebrates;
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = RestHelper.prepareAuthedHttpHeader(token);
        HttpEntity<String> request = new HttpEntity<>(headers);
        try {
            ...
            log.error("Failed to update includeInvertebrates for aquarium_id={}", aquariumId, e);
            throw new BusinessException(CommonExceptionCodes.NETWORK_ERROR);
        }
    }
